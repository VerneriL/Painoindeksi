...
def test_static_get_user_integer(monkeypatch):
    user_input='100'
    monkeypatch.setattr('builtins.input', lambda _:user_input)
    assert questions.Question.get_user_integer('Luku: ', False) == (100, 'OK', 0, 'Conversion successful')

def test_static_get_user_integer_2(monkeypatch):
    user_input='100 v'
    monkeypatch.setattr('builtins.input', lambda _:user_input)
    assert questions.Question.get_user_integer('Luku: ', False) == (0, 'Error', 1, "invalid literal for int() with base 10: '100 v'")
...
def test_static_get_input_float_error(monkeypatch):
    user_input = "74,6"
    monkeypatch.setattr('builtins.input', lambda _:user_input)
    assert questions.Question.get_input_float('Luku: ', False) == (0, 'Error', 1, "could not convert string to float: '74,6'")
